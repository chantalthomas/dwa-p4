...
@section('content')
    ...
    <form method='POST' action='/user-profile' class="calendarEventContainer">
        <div class='createNewEventContainer>'>
            {{ csrf_field() }}
            Title<input type="text" name="title" id='title' value='{{ old('title') }}'>
            @include('modules.field-error', ['field' => 'title'])

            Description<input type="text" name="description" id='description' value='{{ old('description') }}'>
            @include('modules.field-error', ['field' => 'description'])

            Start Date<input type='date' name='startDate' id='startDate' value='{{ old('startDate') }}'>
            @include('modules.field-error', ['field' => 'startDate'])

            End Date<input type='date' name='endDate' id='endDate' value='{{ old('endDate') }}'>
            @include('modules.field-error', ['field' => 'startDate'])
